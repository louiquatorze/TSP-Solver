...
5. **Configure Graph Dependencies:** In `c++/include/structs/TSP.h`, add your algorithm keys to the tracking sets:
   ...
   * `hasHeuristics`
   ...
   For readability, add the algorithm to each of the sets and grey them out based on their mathematical dependencies.
6. **Implement the Logic:** Write your TSP algorithm inside `c++/src/solvers/<your_solver>.cpp`. 
7. **Update Build Files:** Add your new source file (`c++/src/solvers/<your_solver>.cpp`) to the `add_library` target block inside `CMakeLists.txt` so it compiles successfully.
...
2. **Configure Graph Dependencies:** In `python/src/solver/structs/tsp.py` (`TSP` class), add your algorithm keys to the tracking sets:
   * `has_edge_weights`
   * `has_heuristics`
   * `requires_beta`
   
   For readability, add the algorithm to each of the sets and grey them out based on their mathematical dependencies. **This must match the C++ Dependencies**.
